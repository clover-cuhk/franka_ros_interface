...
namespace franka_ros_controllers {
  ...
  bool EffortJointTorqueController::init(hardware_interface::RobotHW* robot_hw,
    ...
    std::string controller_prefix = "EffortJointTorqueController: ";
    ...
    if (!node_handle.getParam("/franka_ros_interface/effort_joint_torque_controller/compensate_coriolis", enable_coriolis)) {
      if (is_left == 1) {
        if (!node_handle.getParam("/panda_left/franka_ros_interface/effort_joint_torque_controller/compensate_coriolis", enable_coriolis)) {
          ROS_ERROR_STREAM(controller_prefix << "Could not read parameter compensate_coriolis for left arm");
          return false;
        }
      } else if (is_left == 0) {
        if(!node_handle.getParam("/panda_right/franka_ros_interface/effort_joint_torque_controller/compensate_coriolis", enable_coriolis)) {
          ROS_ERROR_STREAM(controller_prefix << "Could not read parameter compensate_coriolis for right arm");
          return false;
        }
      } else {
        ROS_ERROR_STREAM(controller_prefix << "Cannot get compensate_coriolis from default or left/right robot config");
        return false;
      }
    }
    ...
    if (!node_handle.getParam("/robot_config/joint_config/joint_effort_limit", torque_limit_map)) {
      if (is_left == 1) {
        if (!node_handle.getParam("/panda_left/robot_config/joint_config/joint_effort_limit", torque_limit_map)) {
          ROS_ERROR_STREAM(controller_prefix << "Cannot get left arm joint effort limit");
          return false;
        }
      } else if (is_left == 0) {
        if (!node_handle.getParam("/panda_right/robot_config/joint_config/joint_effort_limit", torque_limit_map)) {
          ROS_ERROR_STREAM(controller_prefix << "Cannot get right arm joint effort limit");
          return false;
        }
      } else {
        ROS_ERROR_STREAM(controller_prefix << "Cannot get joint effort limit from default or left/right robot config");
        return false;
      }
    }
