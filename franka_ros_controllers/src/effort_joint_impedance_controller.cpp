...
namespace franka_ros_controllers {
  ...
  bool EffortJointImpedanceController::init(hardware_interface::RobotHW* robot_hw,
    ...
    if (prefix.find("left") != std::string::npos) {
      ROS_INFO("EffortJointImpedanceController: Initializing the left arm");
      is_left = 1;
    } else if (prefix.find("right") != std::string::npos) {
      ROS_INFO("EffortJointImpedanceController: Initializing the right arm");
      is_left = 0;
    } else {
      ROS_INFO("EffortJointImpedanceController: Initializing the default arm");
    }
  ...
  void EffortJointImpedanceController::jointCmdCallback(const franka_core_msgs::JointCommandConstPtr& msg) {
    ...
    if (msg->mode == franka_core_msgs::JointCommand::IMPEDANCE_MODE){
      if (msg->position.size() != 7) {
        ...
        ROS_ERROR_STREAM(
            "EffortJointImpedanceController: Commanded positions or velocities are beyond allowed position limits.");
        pos_d_target_ = prev_pos_;
