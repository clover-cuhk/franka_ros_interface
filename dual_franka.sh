...
FRANKA_ROBOT_IP_L="192.168.1.120"
FRANKA_ROBOT_IP_R="192.168.1.121"

# ----- EDIT THIS TO MATCH YOUR IP IN THE NETWORK
your_ip="192.168.1.200"
...
if [ "${1}" == "master" ] || [[ "${1}" == "local" ]]; then
    if [ "${FRANKA_ROBOT_IP_L}" == "FRANKA_ROBOT_IP_L.local" ]; then
    echo -e "\n\t\033[01;31m\033[1mError:\033[00m [franka.sh] \033[01;33m\033[4mFRANKA_ROBOT_IP_L\033[00m variable has to be set to match the IP address of the left Franka Robot Controller\n"
    exit 1
    fi
    if [ "${FRANKA_ROBOT_IP_R}" == "FRANKA_ROBOT_IP_R.local" ]; then
    echo -e "\n\t\033[01;31m\033[1mError:\033[00m [franka.sh] \033[01;33m\033[4mFRANKA_ROBOT_IP_R\033[00m variable has to be set to match the IP address of the right Franka Robot Controller\n"
    exit 1
    fi
    ...
    ROS_MASTER_IP="$your_ip"
    IS_MASTER="true"
    FRANKA_ROBOT_IP_L="sim"
    FRANKA_ROBOT_IP_R="sim"
else
    echo -e "\n\t\033[01;31m\033[1mError:\033[00m [franka.sh] At least one argument has to be passed
...
3) Run this script again upon completion of your workspace build.\n"
    ...
    export FRANKA_ROBOT_IP_R=$FRANKA_ROBOT_IP_R
    export FRANKA_ROBOT_IP_L=$FRANKA_ROBOT_IP_L
    ...
    if [[ $IS_MASTER == true ]]; then
        echo -e "\nLeft ROBOT: $FRANKA_ROBOT_IP_L\n"
        echo -e "\nRight ROBOT: $FRANKA_ROBOT_IP_R\n"
    else
    ...
    __ros_prompt () {
        ...
        if ! echo \${PS1} | grep '\[franka' &>/dev/null; then
            if [[ $your_ip == "localhost" ]]; then
                export PS1="\[\033[00;33m\][franka <SIM> - Robot@\]\[\033[00m\] \${PS1}"
            elif [[ $IS_MASTER == true ]]; then
                export PS1="\[\033[00;33m\][franka <Master> - Robots@\${FRANKA_ROBOT_IP_L} ${FRANKA_ROBOT_IP_R}]\[\033[00m\] \${PS1}"
            else
                export PS1="\[\033[00;33m\][franka <Remote> - Master@\${ROS_MASTER_IP}]\[\033[00m\] \${PS1}"
            fi
        fi
    }
    ...
    if [ "\${TERM}" != "dumb" ]; then
        ...
        if [[ $your_ip == "localhost" ]]; then
            ...
            export PS1="\[\033[00;33m\][franka <Master> - Robot@\${FRANKA_ROBOT_IP_L} ${FRANKA_ROBOT_IP_R}]\[\033[00m\] \${PS1}"
        else
            export PS1="\[\033[00;33m\][franka <Remote> - Master@\${ROS_MASTER_IP}]\[\033[00m\] \${PS1}"
        fi
    fi
